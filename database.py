...
async def set_zaddress(discord_id: int, address: str | None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET zaddress = ? WHERE discord_id = ?",
            (address, discord_id),
        )
        await db.commit()
...
async def set_default_tip(discord_id: int, amount: float):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET default_tip = ? WHERE discord_id = ?",
            (amount, discord_id),
        )
        await db.commit()
...
async def record_tip_count(from_user: int, to_user: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO tip_counts (from_user, to_user) VALUES (?, ?)",
            (from_user, to_user),
        )
        await db.commit()
...
async def get_top_tippers_by_count(limit: int = 10) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT from_user, COUNT(*) as total FROM tip_counts "
            "GROUP BY from_user ORDER BY total DESC LIMIT ?",
            (limit,),
        )
...
async def get_top_receivers_by_count(limit: int = 10) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT to_user, COUNT(*) as total FROM tip_counts "
            "GROUP BY to_user ORDER BY total DESC LIMIT ?",
            (limit,),
        )
        return await cursor.fetchall()
